...
/** Resolve STS separately so a Logs-only endpoint never receives identity requests. */
async function identify(env: Env, region: string | null): Promise<Identity> {
	const client = createStsClient(resolveAwsConfig(env, region, 'sts'));
	try {
...
export class ArchiveLocator {
	...
	async locate(basePath: string, env: Env, selection: ArchiveSelection): Promise<string> {
		...
		const stsEndpoint = resolveAwsConfig(env, selection.region, 'sts').endpoint;
		...
		const key = hash(
			JSON.stringify([
				...
				// Keep existing cache keys when both services share an endpoint.
				...(stsEndpoint && stsEndpoint !== config.endpoint ? [stsEndpoint] : []),
			]),
		);
		...
		try {
			...
			// Only failed lookups may fall back to the last verified account, and
			// only for reads. A successful lookup always replaces stale mappings.
			let cached = selection.readOnly ? this.#known.get(cachePath) : undefined;
			if (selection.readOnly && !cached) {
				try {
					cached = JSON.parse(readFileSync(cachePath, 'utf8')) as Identity;
				} catch {
					/* No verified offline mapping yet. */
				}
			}
			if (!cached || !valid(cached) || (config.region && cached.region !== config.region))
				throw error;
			identity = cached;
		} finally {
		...
		this.#known.set(cachePath, identity);
		try {
